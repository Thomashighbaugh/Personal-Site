...
Using all the same repos that are part of the constellation that are my dotfiles' many composite repos, I was even able to easily roll in the submodules used in my `zsh` configuration without _too much_ fuss. The command to include that repository, in case you were wondering, is:
