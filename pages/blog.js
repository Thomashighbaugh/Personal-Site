...
import PromotedBlog from "../components/BlogRoll/PromotedBlog";
import Pagination from "../layout/Pagination";
import BlogRoll from "../components/BlogRoll/BlogRoll";
import { formatToSlug, queryFromUrl, whitelisted } from "../utils/format";
...
const Blog = ({ allBlogs, pagesArray, promotedBlog }) => {
  ...
  return (
    <Layout title="Blog" id="blog">
      <div className="blog-layout">
        <PageTitle
          title="Blog"
          subtitle="A place to gather my tech related thoughts."
        />
        {currentPage === 1 && promotedBlog && (
          <PromotedBlog blog={promotedBlog} />
        )}
        <BlogRoll allBlogs={blogsPerPage} />
        <Pagination pagesArray={pagesArray} currentPage={currentPage} />
      </div>
    </Layout>
  );
};
...
export async function getStaticProps() {
  ...
  const promotedBlog = allBlogs.find((blog) => blog.document.data.promoted);
