...
import srcs from "~/data/profile.json";
...
const fetchRepos = async ({
  ...
  type,
  ignore,
}: ProjectSrc): Promise<GitHubRepos> => {
  const response = await fetch(
    `https://api.github.com/${type}/${slug}/repos?per_page=100`,
    {
      headers: {
 ...
 * both from my main user and organization accounts
 *
 */
export async function fetchProjects(): Promise<Array<Project> | null> {
  const projects = [];
  for (const source of srcs.projectSrcs as Array<ProjectSrc>) {
