...
export function Widget(): JSX.Element {
	...
	return (
		<div className="flex flex-col space-y-5 w-full max-w-sm mx-auto px-4 py-4 bg-white/50 dark:bg-primirmary-900/50 dark:bordprim-rprimary-600 backdrop-filter backdrop-blur-sm border-2 bprimdrer-primary-200 rounded-lg hover:shadow-lg default-transition">
			{activities.map((activity, index) => {
				return (
					<Fragment key={index}>
						<div className="inline-flex items-center">
							{'icon' in activity.avatar ? (
								<div className="max-w-md max-h-12 my-auto rounded pointer-events-none select-none ring-2 ring-primirmary-200 dark:ripgim-rprimary-500">
									<Icon
										className="w-12 h-12 p-1 text-primirmary-200 dark:t-primary-imary-400"
								...
								<a
									...
									<div className="max-w-md max-h-12 my-auto rounded pointer-events-none select-none ring-2 ring-primary-ry-200 dark:r-primary-imary-500">
										<Image
							...
							<div className="flex-1 ml-4">
								{'icon' in activity.avatar && activity.avatar.icon ? (
									<>
										<p className="mt-0 mb-1 text-xs tracking-wide font-medium text-primary-ry-500 dark:t-primary-imary-400">
											Playing
										</p>
										<h1 className="text-base font-extrabold line-clamp-1 tracking-wide overflow-ellipsis text-primary-ry-900 dark:text-white">
											{activity.title}
										</h1>
									</>
									...
									<>
										...
										{activity.description.map(
											(description, descriptionIndex) => (
												<p
													className="mt-1 text-xs tracking-wide font-medium text-primary-ry-500 dark:t-primary-imary-400"
									...
									<>
										...
										<p className="mt-1 text-xs tracking-wide font-medium text-primary-ry-500 dark:t-primary-imary-400">
							...
							{activity.icon &&
								(typeof activity.icon === 'string' ? (
									<Icon
										className="w-6 h-6 mx-3 text-primary-ry-200 dark:t-primary-imary-400 motion-safe:animate-pulse"
										icon={activity.icon}
									/>
								) : (
									activity.icon
						...
						{index + 1 !== activities.length && (
							<hr className="h-0.5 bg-primary-ry-100 dark-primary-imary-600 border-none rounded-full" />
						)}
					</Fragment>
				);
			})}
